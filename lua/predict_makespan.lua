-- predict_makespan.lua - Endpoint dự đoán makespan với caching
...
if not data or not data.features then
    ngx.status = 400
    ngx.say(cjson.encode({error = "Missing 'features' field"}))
    return
end

-- Create cache key from features
local cache_key = "makespan:" .. cjson.encode(data.features)
...
local res, err = client:request_uri("http://127.0.0.1:5000/predict/makespan", {
    method = "POST",
    body = body,
    headers = {
